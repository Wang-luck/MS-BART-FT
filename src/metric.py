    ...
    def evaluate_de_novo_step_selfies(self, selfies_pred: T.List[str], selfies_true: T.List[str]) -> T.Dict[str, float]:
        ...
        if  len(valid_mols_pred) == 0:
            return {
                ...
                "top1_tanimoto_sim": 0,
                "top1_mol_accuracy": 0,
                "top1_mol_accuracy_stereo_free": 0,
            }
        ...
        # 去除立体化学的匹配率（只看平面结构）
        stereo_free_correct = 0
        for pred_smi, true_smi in zip(valid_smiles_pred, valid_smiles_true):
            try:
                ...
                if pred_mol and true_mol:
                    ...
                    pred_flat = Chem.MolToSmiles(pred_mol, canonical=True)
                    true_flat = Chem.MolToSmiles(true_mol, canonical=True)
                    if pred_flat == true_flat:
                        stereo_free_correct += 1
            except:
                pass
        metric_vals["top1_mol_accuracy_stereo_free"] = stereo_free_correct / len(valid_smiles_pred)
    ...
    def evaluate_de_novo_step_selfies_top_k(self, selfies_pred_topk: T.List[T.List[str]], selfies_true: T.List[str]) -> T.Dict[str, float]:
        ...
        if valid_cnt == 0:
            return {
                ...
                "topk_tanimoto_sim": 0,
                "topk_mol_accuracy": 0,
                "topk_mol_accuracy_stereo_free": 0,
            }
        metric_vals = {
        ...
        for smiles_topk, smiles_true in zip(canonical_smiles_pred_topk, canonical_smiles_true):
            try:
                true_mol = Chem.MolFromSmiles(smiles_true)
                if true_mol is None:
                    continue
                Chem.RemoveStereochemistry(true_mol)
                true_flat = Chem.MolToSmiles(true_mol, canonical=True)
                for pred_smi in smiles_topk:
                    if pred_smi is None:
                        continue
                    pred_mol = Chem.MolFromSmiles(pred_smi)
                    if pred_mol is None:
                ...
                pass
        metric_vals["topk_mol_accuracy_stereo_free"] = stereo_free_correct / len(canonical_smiles_true)
        for key in metric_vals:
            metric_vals[key] = round(metric_vals[key], 4)
