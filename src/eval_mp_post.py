...
if __name__ == "__main__":
    ...
    with distributed_state.split_between_processes(test_dataset, apply_padding=False) as batched_prompts:
        ...
        for i in tqdm(range(0, len(batched_prompts), batch_size), desc=f"Rank [{distributed_state.process_index}] processing: "):
            ...
            # 无论新增什么数据集，只要 TSV 中有 identifier 或 spec 列就自动识别
            sample_ids_name = "identifier" if "identifier" in test_dataset.column_names else "spec" if "spec" in test_dataset.column_names else "name"
    ...
    if distributed_state.is_main_process:
        ...
        save_path = args.output_path if args.output_path else os.path.join("logs", "results", model_name + ".jsonl")
        # Save results
        save_predictions = []
